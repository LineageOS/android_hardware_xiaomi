...
ndk::ScopedAStatus Lights::setLightState(int32_t id, const HwLightState& state) {
    ...
    switch (type) {
        ...
        case LightType::BATTERY:
            LOG(INFO) << "Setting battery light to " << color.toBrightness();
            mLastBatteryState = state;
            updateNotificationColor();
            break;
        case LightType::NOTIFICATIONS:
            LOG(INFO) << "Setting notification light to " << color.toBrightness();
            mLastNotificationsState = state;
            updateNotificationColor();
            break;
        case LightType::ATTENTION:
            LOG(INFO) << "Setting attention light to " << color.toBrightness();
