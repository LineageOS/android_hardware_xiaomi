...
ndk::ScopedAStatus Lights::setLightState(int32_t id, const HwLightState& state) {
    switch (id) {
        case (int32_t)LightType::BACKLIGHT:
            if (!mBacklightPath.empty())
                writeToFile(mBacklightPath, colorToBrightness(state.color));
            break;
        case (int32_t)LightType::BUTTONS:
            for (auto& buttons : mButtonsPaths)
                writeToFile(buttons, isLit(state.color));
            break;
        case (int32_t)LightType::BATTERY:
        case (int32_t)LightType::NOTIFICATIONS:
            mLEDMutex.lock();
            if (id == (int32_t)LightType::BATTERY)
