...
ndk::ScopedAStatus Lights::setLightState(int32_t id, const HwLightState& state) {
    ...
    switch (type) {
        case LightType::BACKLIGHT:
            if (mBacklightDevice)
                mBacklightDevice->setBrightness(color.toBrightness());
            break;
        ...
        case LightType::NOTIFICATIONS:
            ...
            rgb_t batteryState(mLastBatteryState.color);
            setLED(new batteryState.isLit() ? mLastBatteryState : mLastNotificationState);
            mLEDMutex.unlock();
            break;
        default:
