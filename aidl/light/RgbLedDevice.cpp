...
RgbLedDevice::RgbLedDevice(LedDevice red, LedDevice green, LedDevice blue, std::string rgbSync)
    : mRed(red), mGreen(green), mBlue(blue), mRgbSync(rgbSync), mColors(Color::NONE) {
    if (mRed.exists()) {
        mColors |= Color::RED;
    }
    if (mGreen.exists()) {
...
bool RgbLedDevice::supportsRgbSync() const {
    return std::ifstream(mRgbSync).good();
}
