...
const GetConnectStateResult parseGetConnectStateResponse(const std::array<uint8_t, 64>& response) {
    GetConnectStateResult get_connect_state_result;

    get_connect_state_result.success = response[4] != kConnectStateSuccess;

    if (!get_connect_state_result.success) {
        return get_connect_state_result;
    }

    get_connect_state_result.voltage = (response[11] << 8) | response[10];
    get_connect_state_result.clock_off = (response[13] << 8) | response[12];
    get_connect_state_result.true_off =
            (response[17] << 24) | (response[16] << 16) | (response[15] << 8) | response[14];

    if (response[18] == 1) {
        get_connect_state_result.connect_state = ConnectState::OVERCHARGED;
    } else if ((response[9] & 0x03) == 0x01) {
        get_connect_state_result.connect_state = ConnectState::TRX_CHECK_FAILED;
    } else if ((response[9] & 0x63) == 0x43) {
        get_connect_state_result.connect_state = ConnectState::PIN_CONNECT_FAILED;
    } else if ((response[9] & 0x03) == 0x00) {
        get_connect_state_result.connect_state = ConnectState::DISCONNECTED;
    } else if ((response[9] & 0x63) == 0x23) {
        get_connect_state_result.connect_state = ConnectState::CONNECTED;
    } else {
        get_connect_state_result.connect_state = ConnectState::UNKNOWN;
    }
