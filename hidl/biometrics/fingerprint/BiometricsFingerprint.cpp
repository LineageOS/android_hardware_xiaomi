...
void xiaomi_udfps_init() {
    ...
    if (xiaomi_udfps_handle) {
        ...
        udfpsThread =
                (udfpsThread_t)dlsym(xiaomi_udfps_handle, "_Z11udfpsThreadP18fingerprint_device");
        if (udfpsThread == NULL) {
            ALOGE("%s: dlsym error for udfpsThread: %s", __func__, dlerror());
            goto udfps_error;
        }

        xiaomiOnFingerDown = (xiaomiOnFingerDown_t)dlsym(
                xiaomi_udfps_handle, "_Z18xiaomiOnFingerDownP18fingerprint_devicejjff");
        if (xiaomiOnFingerDown == NULL) {
            ALOGE("%s: dlsym error for xiaomiOnFingerDown: %s", __func__, dlerror());
            goto udfps_error;
        }

        xiaomiOnFingerUp = (xiaomiOnFingerUp_t)dlsym(xiaomi_udfps_handle,
                                                     "_Z16xiaomiOnFingerUpP18fingerprint_device");
